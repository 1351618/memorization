...
function outputToPage(data) {
          ...
          (_, i) => {
            ...
            const listItems = Object.keys(data)
              ...
              .map((key, index) => {
                ...
                ${numbOfTransl
                  .map((val) => {
                    return `<p class="lang-${val}">${data[key].lang[val]}</p>`;
                  })
                  .join("")}
              </li>${br}`;
              })
  ...
  window.updateCheckboxState = function (language, isChecked) {
    checkboxStates[language] = isChecked;
    updateLanguagesDisplay(language, isChecked);
  };

  // Функция для обновления отображения языков
  function updateLanguagesDisplay(language, isChecked) {
